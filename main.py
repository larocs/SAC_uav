...
class SAC():
    ...
    def __init__(self,env, replay_buffer_size, hidden_dim, restore_path, device,max_episodes, save_path,
        ...
        if self.use_double:
            self.state_to_tensor = lambda x : torch.DoubleTensor(x).unsqueeze(0).to(device)
        else:
            self.state_to_tensor = lambda x : torch.FloatTensor(x).unsqueeze(0).to(device)
    ...
    def soft_q_update(self, 
        ...
        if self.use_double:
            ...
            done       = torch.DoubleTensor(np.float64(done)).unsqueeze(1).to(self.device)
    ...
    def __write_csv(self, episode,time_elapsed , frame_count, len_replay_buffer, episode_reward, \
        ...
        with open(os.path.join(self.save_path, 'progress.csv'), 'a') as csvfile:
            ...
            rew_writer.writerow([episode,time_elapsed , frame_count, len(self.replay_buffer), episode_reward, \
                value_loss, q_value_loss, policy_loss, step])
        ...
        while episode < max_episodes:
            ...
            for step in range(time_horizon):
                if frame_count > self.min_num_steps_before_training: 
                    ...
                    action = self.policy_net.get_action(self.state_to_tensor(state))# .detach()
                    next_state, reward, done, self.env_info = self.env.step(action*self.action_range[1])
                ...
                if len(self.replay_buffer) > batch_size:
                    if (episode % args.num_steps_until_train ) == 0:
                        for i in range(args.num_trains_per_step):
                            q_value_loss, policy_loss, value_loss = self.soft_q_update(batch_size)
            ...
            if (episode % 100 == 0) and (episode>0):
                print('Time elapsed so far = {0:.2f} seconds.'.format(time_elapsed))
